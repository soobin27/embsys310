...
delay
    ...
    CMP         R0, #0          // Check if the user passed zero
    BEQ         exit            // Exit if value is zero
    CMP         R0, #0          // Compare the passed value with zero
    BLE         exit            // Exit if negative value
countdown    
    SUB         R0, R0, #1      // substract 1 from passed value
    CMP         R0, #0          // compare if value is zero
    BNE         countdown       // branch to 'countdown' if R0 is not zero
exit
