...
I would like to discuss the Ring security contact sensor. This device is installed on a door or window and it senses when a door or window is open or closed. This device then sends the status to a base station that connects to the cloud and provides an update to an app on the user's mobile device.
